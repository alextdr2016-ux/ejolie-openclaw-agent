...
class ReportGenerator:
    ...
    def sales_report(self, start_date: str, end_date: str, status_id: int = None) -> str:
        ...
        for order_id, order in orders.items():
            ...
            if 'produse' in order:
                for product in order['produse']:
                    # Calculate product total
                    quantity = int(product.get('cantitate', 0))
                    price = float(product.get('pret', 0))
                    product_total = quantity * price
                    total_sales += product_total
                    total_items += quantity

                    # Track products sold
                    product_key = f"{product.get('nume', 'Unknown')} (Marime: {product.get('marime', 'N/A')})"
                    products_sold[product_key] = products_sold.get(
                        product_key, 0) + quantity
...
📦 **Produse vândute:**
- Total articole: {total_items} bucăți
